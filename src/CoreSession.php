...
class CoreSession implements Session
{
  ...
  public function &getNamedSection(string $name, int $mode)
  {
    if (!isset($this->sections[$name]))
    {
      ...
      {
        $section = Nub::$DL->abcAuthSessionNamedSectionGet(Nub::$companyResolver->getCmpId(),
                                                           $this->session['ses_id'],
                                                           $name,
                                                           $mode);
      }
  ...
  public function login(int $usrId): void
  {
    ...
    $this->session = Nub::$DL->abcAuthSessionLogin($this->session['cmp_id'],
                                                   $this->session['ses_id'],
                                                   $usrId,
                                                   self::getRandomToken(),
                                                   self::getRandomToken());
  ...
  public function logout(): void
  {
    ...
    $this->session = Nub::$DL->abcAuthSessionLogout($this->session['cmp_id'],
                                                    $this->session['ses_id'],
  ...
  public function save(): void
  {
    ...
    Nub::$DL->abcAuthSessionUpdateSession($this->session['cmp_id'], $this->session['ses_id'], $serial);
  ...
  public function setLanId(int $lanId): void
  {
    ...
    Nub::$DL->abcAuthSessionUpdateLanId($this->session['cmp_id'], $this->session['ses_id'], $lanId);
  }
  ...
  public function start(): void
  {
    ...
    {
      // Start a new session.
      $this->session = Nub::$DL->abcAuthSessionStartSession(Nub::$companyResolver->getCmpId(),
                                                            Nub::$languageResolver->getLanId(),
                                                            self::getRandomToken(),
                                                            self::getRandomToken());
    }
    ...
    {
      $this->session = Nub::$DL->abcAuthSessionGetSession(Nub::$companyResolver->getCmpId(), $sesSessionToken);
  ...
  private function saveNamedSection(string $name, $data): void
  {
    if ($data===null)
    {
      Nub::$DL->abcAuthSessionNamedSectionDelete(Nub::$companyResolver->getCmpId(),
                                                 $this->session['ses_id'],
                                                 $name);
    }
    else
    {
      Nub::$DL->abcAuthSessionNamedSectionUpdate(Nub::$companyResolver->getCmpId(),
                                                 $this->session['ses_id'],
                                                 $name,
                                                 serialize($data));
    }
